...
instance : semiring (aux p) := {
  ...
  ..(infer_instance : has_add (aux p)),
  ..(infer_instance : has_mul (aux p)),
  ..(infer_instance : has_one (aux p)),
  ..(infer_instance : has_zero (aux p)), }
...
begin
  ...
  -- there must be a tactic for `f x = g x → f = g`, right?
  suffices : alg_hom.id R (free_algebra R X) = (aux.value_hom p).comp (lift R (aux.of p)), { rw this, },
  ext,
